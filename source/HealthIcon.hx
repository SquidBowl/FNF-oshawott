...
class HealthIcon extends FlxSprite
{
	...
	public function changeIcon(char:String) {
		if(this.char != char) {
			...
			loadGraphic(file); // load this first to get file size
			
			splitThing = width/3 == height ? 3 : 2;
			loadGraphic(file, true, Math.floor(width / splitThing), Math.floor(height)); //Then load it fr // winning icons go br
